...
def cadastro_medico(request):
    ...
    if is_medico(request.user):
        messages.add_message(request, constants.WARNING, "Você já possui cadastro médico")
...
def abrir_horario(request):
    if not is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Cadastro médico não efetuado, não é possível abrir horários para consulta')
